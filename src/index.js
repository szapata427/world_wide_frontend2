 ...
 .then(function(json){
  ...
  let marker = new google.maps.Marker({
   ...
   marker.addListener('mouseover', function(event) {
     let countryPopulationone = Number(parseFloat(country["population"]).toFixed(2)).toLocaleString('en', {
    ...
    infoWindow = new google.maps.InfoWindow({
     content: `<div class="country-info"><h4 class="country-title">${country["name"]}<h4>
          ...
          <p><span class="info-object">Population:</span> <span class="info-style">${countryPopulationone}</span></p>
...
fetch(`http://localhost:3000/countries`)
  ...
  .then(json => {
    json.forEach(country => {
      datalistOptions.innerHTML += `<option value="${country.name}"><span id="${country.id}"></span>`
      search.addEventListener('submit', repop)
     })
    })
// })

function repop(event){
  ...
  let input = document.querySelector('input')
    ...
    fetch('http://localhost:3000/countries')
      ...
      .then(function(json){
     // console.log(json)
     let foundCountry = json.find(function(country){
       return country.name == input.value
     })

     let map = new google.maps.Map(document.getElementById('map'), {
        center: {lat: foundCountry['location_lat'], lng: foundCountry['location_lng']},
        zoom: 4
       });
  ...
  // if (input.value === country.name){
   ...
   foundMarker = new google.maps.Marker({
     position:{ lat: foundCountry['location_lat'] ,lng: foundCountry['location_lng'] },
     map:map,
  ...
  foundMarker.addListener('click', function(event) {
    let countryPopulation = Number(parseFloat(foundCountry["population"]).toFixed(2)).toLocaleString('en', {
     minimumFractionDigits: 0
   })

   // create a new infoWindow for the foundMarker

     if (infoWindow) infoWindow.close();
     infoWindow = new google.maps.InfoWindow({
      content: `<div class="country-info"><h4 class="country-title">${foundCountry["name"]}<h4>
           <p><span class="info-object"> Native Name:</span> <span class="info-style">${foundCountry["native_name"]}</span></p>
           <p><span class="info-object">Population:</span> <span class="info-style">${countryPopulation}</span></p>
           <p><span class="info-object">Official Language:</span> <span class="info-style">${foundCountry["language"]}</span></p>
